...
public class SampleController implements Initializable {
	...
	@FXML
	protected void saveCSVPressed(ActionEvent e) throws Exception {
		FileChooser fileChooser = new FileChooser();
		fileChooser.setTitle("Select csv file to save");
		fileChooser.getExtensionFilters().add(new ExtensionFilter("Comma Separated values", "*.csv"));
		File outputFile = fileChooser.showSaveDialog(saveCSVButton.getScene().getWindow());
		if (outputFile == null) {
			return;
		}
	...
	@FXML
	protected void stepButtonPressed(ActionEvent e) throws Exception {
		int step = Integer.parseInt(stepSizeField.getText());
		clearCoordinate();
		drawAxis();
		redrawPoint();
		stepPolynomialRegression(step);
	}
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		group.selectedToggleProperty().addListener((ObservableValue<? extends Toggle> ov, Toggle old_toggle,
				Toggle new_toggle) -> handleRadioButtonSelected(ov, old_toggle, new_toggle));
		drawAxis();
	}
	...
	private void stepPolynomialRegression(int step) {
		final int order;
		int miniBatch;
		GradientDescent gd;
		IterativeMethod iter;
		if (pRegressor != null) {
			pRegressor.stepRegress(step);
			for (double x = -CANVAS_WIDTH / 2; x < CANVAS_WIDTH / 2; x += 1.0e-1) {
				double y = pRegressor.calcRecall(x);
				plotPoint(new Point(x, y));
			}
